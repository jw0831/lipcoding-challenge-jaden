...
def test_match_request_flow():
    ...
    with app.test_client() as client:
        ...
        # Login both users
        mentor_login = client.post('/api/login',
                                 data=json.dumps({
                                     'email': 'mentor@test.com',
                                     'password': 'password123'
                                 }),
        ...
        mentee_login = client.post('/api/login',
                                 data=json.dumps({
                                     'email': 'mentee@test.com',
                                     'password': 'password123'
                                 }),
        ...
        request_response = client.post('/api/match-requests',
                                     data=json.dumps({
        ...
        mentee_requests = client.get('/api/match-requests/outgoing',
                                   headers={'Authorization': f'Bearer {mentee_token}'})
        ...
        mentor_requests = client.get('/api/match-requests/incoming',
        ...
        accept_response = client.put(f'/api/match-requests/{request_id}/accept',
                                   headers={'Authorization': f'Bearer {mentor_token}'})
...
def test_unauthorized_match_request():
    ...
    with app.test_client() as client:
        ...
        request_response = client.post('/api/match-requests',
                                     data=json.dumps({
                                         'mentorId': 1,  # camelCase per API spec
        ...
        assert 'Only mentees can send' in data['error'] or 'Only mentees can send requests' in data['error']
